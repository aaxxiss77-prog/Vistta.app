...
import { db, auth, firebaseConfig, functions, firebaseConfigStatus } from '../config/firebase';
import { Produto, Cliente, Venda, Caixa, CarrinhoItem, Orcamento, OrdemServico } from '../types';

const provisioningApp = firebaseConfigStatus.valid
  ? getApps().find(currentApp => currentApp.name === 'vistta-user-provisioning') || initializeApp(firebaseConfig, 'vistta-user-provisioning')
  : null;
const provisioningAuth = provisioningApp ? getAuth(provisioningApp) : auth;
const provisioningDb = provisioningApp ? getDatabase(provisioningApp) : db;
