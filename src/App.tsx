...
function FirebaseConfigurationScreen() {
  return (
    <main className="flex min-h-[100dvh] items-center justify-center bg-[#f5f6f4] p-6 text-center">
      <section className="w-full max-w-lg rounded-3xl bg-white p-8 shadow-lg">
        <h1 className="mb-3 text-2xl font-bold text-slate-900">Configuração do sistema indisponível</h1>
        <p className="mb-6 text-sm leading-6 text-slate-600">Não foi possível iniciar o VISTTA. A configuração do ambiente está incompleta.</p>
        {import.meta.env.DEV && <p className="mb-6 rounded-xl bg-amber-50 p-4 text-left text-xs text-amber-800">Verifique as variáveis de ambiente do ambiente de implantação.</p>}
        <button onClick={() => window.location.reload()} className="rounded-xl bg-[var(--vistta-plum)] px-5 py-3 font-bold text-white hover:bg-[var(--vistta-violet)]">Tentar novamente</button>
      </section>
    </main>
  );
}
...
export default function App() {
  if (!firebaseConfigStatus.valid) return <FirebaseConfigurationScreen />;
