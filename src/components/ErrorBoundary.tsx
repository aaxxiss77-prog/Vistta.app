...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error('Erro de renderização do VISTTA:', error.name, import.meta.env.DEV ? info.componentStack : undefined);
  }
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 p-4 sm:p-6 text-center">
        <div className="max-w-md rounded-3xl bg-white p-8 shadow-lg">
          ...
          {import.meta.env.DEV && <p className="mb-6 break-words rounded-xl bg-rose-50 p-3 text-left text-xs text-rose-700">{this.state.message}</p>}
